...
public class SunmiPrintHelper {
    ...
    public void printContents(List<PrintContent> contents) {
        if(sunmiPrinterService == null){
            //TODO Service disconnection processing
            return ;
        }
        ...
        try {
            ...
            for (int i = 0; i < contents.size(); i++) {
                PrintContent pc = contents.get(i);
                switch (pc.getTag().toLowerCase()){
                    case "br":
                        sunmiPrinterService.sendRAWData(ESCUtil.nextLine(1), null);
                        break;
                    case "cut":
                        sunmiPrinterService.cutPaper(null);
                        break;
                    case "c":
                        ...
                        sunmiPrinterService.printTextWithFont(pc.getContent(), null, 24, null);
                        break;
                    case "cb":
                        sunmiPrinterService.sendRAWData(ESCUtil.boldOn(), null);
                        sunmiPrinterService.setAlignment(1, null);
                        sunmiPrinterService.printTextWithFont(pc.getContent(), null, 36, null);
                        sunmiPrinterService.sendRAWData(ESCUtil.boldOff(), null);
                        break;
                    case "b":
                    ...
                    case "l":
                        sunmiPrinterService.setAlignment(0, null);
                        sunmiPrinterService.sendRAWData(ESCUtil.doubleHeight(), null);
                        sunmiPrinterService.printTextWithFont(pc.getContent(), null, 24, null);
                        sunmiPrinterService.sendRAWData(ESCUtil.doubleHeightDisable(), null);
                    case "w":
                        sunmiPrinterService.setAlignment(0, null);
                        sunmiPrinterService.sendRAWData(ESCUtil.doubleWidth(), null);
                        sunmiPrinterService.printTextWithFont(pc.getContent(), null, 24, null);
                        sunmiPrinterService.sendRAWData(ESCUtil.doubleWidthDisable(), null);
                        break;
                    case "qr":
                        sunmiPrinterService.setAlignment(1, null);
                        sunmiPrinterService.printQRCode(pc.getContent(), 10, 0, null);
                        sunmiPrinterService.sendRAWData(ESCUtil.nextLine(1), null);
                        break;
                    case "bar":
                        sunmiPrinterService.setAlignment(1, null);
                        sunmiPrinterService.printBarCode(pc.getContent(),  8, 90, 2, 2, null);
                        break;
                    case "tr":
                        String[] lineContents = pc.getContent().split("#");
                        if (lineContents.length == 2) {
                            int width[] = new int[]{1, 1};
                            int align[] = new int[]{0, 2};
                            sunmiPrinterService.printColumnsString(lineContents, width, align, null);
                        }
                        break;
                    case "th":
                        String[] lineHead = pc.getContent().split("#");
                        if (lineHead.length == 2) {
                            int width[] = new int[]{1, 1};
                            int align[] = new int[]{0, 2};
                            sunmiPrinterService.sendRAWData(ESCUtil.boldOn(), null);
                            sunmiPrinterService.printColumnsString(lineHead, width, align, null);
                            sunmiPrinterService.sendRAWData(ESCUtil.boldOff(), null);
                        }
                        break;
                    case "line":
